    ...
    <div class="container">
      <div class="card">
        ...
        <form (submit)="onSubmit()">
          <input type="email" name="email" [(ngModel)]="ngModelEmail" placeholder="Email" required>
          <input type="password" name="password" [(ngModel)]="ngModelPassword" placeholder="Contraseña" required>
          @if(error()) { <p class="error">{{ error() }}</p> }
          <button type="submit" [disabled]="loading()">{{ loading() ? 'Cargando...' : 'Entrar' }}</button>
        </form>
        <p class="footer">¿No tienes cuenta? <a routerLink="/register">Regístrate</a></p>
      </div>
    </div>
  `,
  styles: [`
    .container { height: 100vh; display: flex; align-items: center; center; justify-content: center; background: #0f172a; color: white; }
...
export class LoginComponent {
  ...
  onSubmit() {
    ...
    this.authService.login({ email: this.ngModelEmail, password: this.ngModelPassword }).subscribe({
